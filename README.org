...
Use =flyspell-correct-region= to correct all misspelled words in the selected
region. This command first runs =flyspell-region= to identify misspelled words,
then lets you correct them one by one.

#+begin_src emacs-lisp
  (define-key flyspell-mode-map (kbd "C-c r") #'flyspell-correct-region)
#+end_src

This is useful for spell-checking a specific portion of the buffer (e.g., a
paragraph or code comment) without being prompted about errors elsewhere.
...
By default, =flyspell-correct-wrapper= searches for misspelled words backward
from point. You can change this by setting =flyspell-correct-default-direction=
to =forward=.

#+begin_src emacs-lisp
  (setq flyspell-correct-default-direction 'forward)
#+end_src

Note that =C-u C-u= prefix still toggles the direction regardless of this
setting.

** Quit behavior

By default, pressing =C-g= during correction restores point to its original
position. If you prefer to stay at the misspelled word when quitting (similar to
the =stop= action), set =flyspell-correct-abort-on-quit= to =nil=.

#+begin_src emacs-lisp
  (setq flyspell-correct-abort-on-quit nil)
#+end_src
...
For users who prefer a hydra-based workflow for spell checking, here is an
example configuration. This hydra allows you to navigate between misspelled
words and correct them on demand, which is useful for reviewing an entire buffer.

#+begin_src emacs-lisp
  (defhydra flyspell-correct-hydra (:color pink :hint nil)
    "
  Spell check:
  _n_: next error    _c_: correct      _b_: check buffer
  _p_: prev error    _a_: accept       _q_: quit
  "
    ("n" flyspell-goto-next-error)
    ("p" (flyspell-check-previous-highlighted-word t))
  ...
  (define-key flyspell-mode-map (kbd "C-c $") #'flyspell-correct-hydra/body)
#+end_src

This approach differs from =flyspell-correct-wrapper= in that you explicitly
navigate to each misspelled word and decide whether to correct it, rather than
being prompted automatically for each error.
...
=flyspell-correct-previous= after certain delay configured by
...
=flyspell-correct-previous= called by =flyspell-correct-auto-mode= by
